...
void Qwen3Model::init_mem() {
  ...
  // Prefill buffers - these are allocated to max sequence length for reuse
  // Prefill RMS output [seq_len, hidden_dim]
  ...
  // Prefill attention output (after wo projection) [seq_len, hidden_dim]
...
base::Status Qwen3Model::prefill_predict(const tensor::Tensor& input, int32_t token_num,
                                           int32_t& next) const {
...
base::Status Qwen3Model::prefill_forward(const tensor::Tensor& input, int32_t token_num,
                                           int32_t& next) const {
  if (input.is_empty()) {
    return base::error::InvalidArgument("The input tensor is empty.");
  }
  ...
  // For prefill, we only need the last token's logits for next token prediction
  // Apply final RMSNorm and LM head only to the last token
  {
    CHECK(qwen_layers_ != nullptr);
    ...
    tensor::Tensor last_token_input(base::DataType::kDataTypeFp32, config_->hidden_dim_, false,
                                    nullptr, const_cast<float*>(input.ptr<float>()) +
                                                  (token_num - 1) * config_->hidden_dim_);
...
void Qwen3Model::attention_rms_prefill(int32_t layer_idx, const tensor::Tensor& input,
                                         int32_t token_num) const {
  CHECK(qwen_layers_ != nullptr);
  std::shared_ptr<op::Layer> rmsnorm_layer = qwen_layers_->rmsnorm_layers_.at(layer_idx);
  CHECK(rmsnorm_layer) << "The attention rmsnorm layer is a null pointer in the qwen3 model";

  // Process each token through rmsnorm separately
  // The rmsnorm output buffer is reused for each token
  // We store all tokens' rmsnorm output in the input tensor reshaped as [token_num, hidden_dim]
  // Actually, we process tokens in-place and store results in prefill buffer
  tensor::Tensor rmsnorm_output = get_buffer(ModelBufferType::kPrefillRMSOutput);
  if (rmsnorm_output.is_empty() || rmsnorm_output.size() < static_cast<size_t>(token_num * config_->hidden_dim_)) {
    LOG(FATAL) << "Prefill RMS output buffer is not properly allocated.";
  }

  for (int32_t t = 0; t < token_num; ++t) {
    tensor::Tensor token_input(base::DataType::kDataTypeFp32, config_->hidden_dim_, false, nullptr,
                                const_cast<float*>(input.ptr<float>()) + t * config_->hidden_dim_);
    token_input.set_device_type(device_type_);

    tensor::Tensor token_output(base::DataType::kDataTypeFp32, config_->hidden_dim_, false, nullptr,
                                 const_cast<float*>(rmsnorm_output.ptr<float>()) +
                                               t * config_->hidden_dim_);
...
void Qwen3Model::attention_qkv_prefill(int32_t layer_idx, int32_t token_num,
  ...
  // For each token, compute wq, wk, wv projections
  const auto& query_layer = qwen_layers_->wq_layers_.at(layer_idx);
  const auto& key_layer = qwen_layers_->wk_layers_.at(layer_idx);
  const auto& value_layer = qwen_layers_->wv_layers_.at(layer_idx);
  ...
  for (int32_t t = 0; t < token_num; ++t) {
    ...
    tensor::Tensor rms_out_t(base::DataType::kDataTypeFp32, config_->hidden_dim_, false, nullptr,
                              const_cast<float*>(rmsnorm_output.ptr<float>()) + t * config_->hidden_dim_);
    ...
    // Query norm
    auto query_norm = qwen_layers_->rmsnorm_layers_.at(layer_idx + 2 * config_->layer_num_ + 1);
    query_t.reshape({(int32_t)query_t.size() / config_->head_size_, config_->head_size_});
    query_norm->forward(query_t, query_t);
    query_t.reshape({(int32_t)query_t.size()});
    ...
    // Key norm
    auto key_norm = qwen_layers_->rmsnorm_layers_.at(layer_idx + 3 * config_->layer_num_ + 1);
    key_t.reshape({(int32_t)key_t.size() / config_->head_size_, config_->head_size_});
    key_norm->forward(key_t, key_t);
    key_t.reshape({(int32_t)key_t.size()});
...
void Qwen3Model::attention_mha_prefill(int32_t layer_idx, int32_t token_num) const {
  CHECK(qwen_layers_ != nullptr);
  ...
  for (int32_t t = 0; t < token_num; ++t) {
    ...
    tensor::Tensor attn_out_t(base::DataType::kDataTypeFp32, config_->hidden_dim_, false, nullptr,
                               const_cast<float*>(attn_output.ptr<float>()) + t * config_->hidden_dim_);
...
void Qwen3Model::feed_forward_prefill(int32_t layer_idx, const tensor::Tensor& input,
                                        int32_t token_num) const {
  CHECK(qwen_layers_ != nullptr);
  ...
  // Process each token through the feed-forward network
  for (int32_t t = 0; t < token_num; ++t) {
    // Residual add: input += attn_output
    tensor::Tensor input_t(base::DataType::kDataTypeFp32, config_->hidden_dim_, false, nullptr,
                            const_cast<float*>(input.ptr<float>()) + t * config_->hidden_dim_);
    ...
    // FFN RMSNorm
    tensor::Tensor ffn_norm_output(base::DataType::kDataTypeFp32, config_->hidden_dim_);
    ...
    // W1
    tensor::Tensor w1_output(base::DataType::kDataTypeFp32, config_->immediate_dim_);
    const auto& w1_layer = qwen_layers_->w1_layers_.at(layer_idx);
    CHECK_NE(w1_layer, nullptr);
    STATUS_CHECK(w1_layer->forward(ffn_norm_output, w1_output));

    // W3
    tensor::Tensor w3_output(base::DataType::kDataTypeFp32, config_->immediate_dim_);
    ...
    // W2
    tensor::Tensor w2_output(base::DataType::kDataTypeFp32, config_->hidden_dim_);
