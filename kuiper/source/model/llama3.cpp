...
base::Status LLama2Model::prefill_predict(const tensor::Tensor& input, int32_t token_num,
                                           int32_t& next) const {
...
base::Status LLama2Model::prefill_forward(const tensor::Tensor& input, int32_t token_num,
                                           int32_t& next) const {
  if (input.is_empty()) {
    return base::error::InvalidArgument("The input tensor is empty.");
  }
...
void LLama2Model::attention_rms_prefill(int32_t layer_idx, const tensor::Tensor& input,
                                          int32_t token_num) const {
  CHECK(llama_layers_ != nullptr);
  std::shared_ptr<op::Layer> rmsnorm_layer = llama_layers_->rmsnorm_layers_.at(layer_idx);
  CHECK(rmsnorm_layer) << "The attention rmsnorm layer is a null pointer in the llama2 model";
...
void LLama2Model::feed_forward_prefill(int32_t layer_idx, const tensor::Tensor& input,
                                           int32_t token_num) const {
  CHECK(llama_layers_ != nullptr);
  ...
  for (int32_t t = 0; t < token_num; ++t) {
    ...
    STATUS_CHECK(llama_layers_->add_layer_->forward(input_t, attn_out_t, input_t));
    ...
    // Residual add
    CHECK_NE(llama_layers_->add_layer_, nullptr);
    STATUS_CHECK(llama_layers_->add_layer_->forward(input_t, w2_output, input_t));
  }
}
